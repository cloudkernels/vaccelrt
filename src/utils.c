...
int read_file(const char *path, void **data, size_t *size)
{
	...
	ret = fstat(fd, &stat);
	if (ret < 0) {
		vaccel_debug("Could not fstat file %s: %s", path, strerror(errno));
		ret = errno;
		goto close_file;
	}
	if (!stat.st_size) {
	...
	while (bytes) {
		...
		if (ret < 0) {
			vaccel_debug("Could not read file %s: %s", path, strerror(errno));
			free(buff);
			ret = errno;
			goto close_file;
		}
...
int read_file_mmap(const char *path, void **data, size_t *size)
{
	...
	void *ptr = mmap(NULL, stat.st_size, PROT_READ|PROT_WRITE, MAP_PRIVATE,
			fd, 0);
	if (!ptr) {
		vaccel_debug("Could not mmap file %s: %s", path, strerror(errno));
		ret = VACCEL_ENOMEM;
		goto close_file;
	}
