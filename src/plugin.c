...
void *get_plugin_op(enum vaccel_op_type op_type, unsigned int hint)
{
	...
	if (VACCEL_REMOTE & hint) {
		for_each_container_safe(opiter, tmp, &plugin_state.ops[op_type],
			...
			if (opiter->owner->info->is_virtio) {
				op = opiter;
				vaccel_debug("Returning func from hint plugin %s ", opiter->owner->info->name);
				goto out;
			}
		}
		vaccel_error("Could not return func, no VirtIO plugin loaded yet");
		return NULL;
	}
	...
	if (!op) {
		...
		vaccel_error("Could not return func, no local plugin loaded yet");
		return NULL;
	}
